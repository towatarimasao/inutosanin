...
// 対象は is_active=true AND listing_status=published AND google_place_id IS NOT NULL
// AND photo_url が googleusercontent.com（Google直リンク）のスポット。
// Google写真をダウンロードしてSupabase Storage(spot-photos)に再アップロードし、
// photo_urlをSupabase Storageの公開URLに更新する。
// photo_urlが更新された時点で対象外になるため、再実行しても同じレコードは再処理されない。
// 1回の実行件数は limit パラメータで制御する（デフォルト20、最大30）。
...
export async function GET(request: Request) {
  ...
  const { data, error: fetchError } = await supabase
    ...
    .ilike("photo_url", "%googleusercontent.com%")
    .limit(limit);
  ...
  console.log(
    `[rehost-google-photos] 対象件数: ${targets.length}件 (limit=${limit}, dryRun=${!execute})`
  );

  if (!execute) {
    // dry-runモード：対象件数と一覧をログ出力するのみ。書き込み・API呼び出しは一切行わない。
    for (const spot of targets) {
      console.log(`[rehost-google-photos][dry-run] 対象: ${spot.name} (id=${spot.id})`);
    }
    return NextResponse.json({
      ...
      targets: targets.map((s) => ({ id: s.id, name: s.name, category: s.category })),
  ...
  for (const spot of targets) {
    try {
      ...
      const path = `${spot.category}/${spot.id}.${ext}`;
      ...
      console.log(`[rehost-google-photos][OK] ${spot.name} (id=${spot.id}) -> ${publicUrlData.publicUrl}`);
      results.push({ id: spot.id, name: spot.name, ok: true });
      succeeded++;
    } catch (err) {
      const message = String(err instanceof Error ? err.message : err);
      console.error(`[rehost-google-photos][NG] ${spot.name} (id=${spot.id}): ${message}`);
  ...
  console.log(
    `[rehost-google-photos] 完了: 成功${succeeded}件 / 失敗${failed}件 / 対象${targets.length}件`
  );
